...
WINDOW - Go ->Powershell->wsl --install ->Ubuntu version install and set user and password for access .After install you can user linux distribution in search menu to run or use cmd 

### Docker Command to Run Ubuntu Linux Container (Persistent & Long-Term) (WINDOWS)
Just install Docker desktop, run the below command and create linux container of any distribution without worrying.After create container user command for access container (docker exec -it <container id> /bin/bash)
